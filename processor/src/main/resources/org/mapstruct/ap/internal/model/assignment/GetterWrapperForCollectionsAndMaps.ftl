...
if ( ${ext.targetBeanName}.${ext.targetWriteAccessorName}<@lib.handleWriteAccesing /> != null ) {
    <@lib.handleExceptions>
      <#if ext.existingInstanceMapping>
        <#if ext.targetType.protobufPrimitiveCollectionOrMapType &&  
        ( "${ext.targetWriteAccessorName}" == "get${ext.targetPropertyName?cap_first}Map" ||  "${ext.targetWriteAccessorName}" == "get${ext.targetPropertyName?cap_first}List" ) >
