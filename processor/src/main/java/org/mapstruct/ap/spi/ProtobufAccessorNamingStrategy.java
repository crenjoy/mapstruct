...
public class ProtobufAccessorNamingStrategy extends DefaultAccessorNamingStrategy {
    ...
    public boolean isGetterMethod(ExecutableElement method) {
        ...
        if (getterMethod && isProtoBufMethod(method) && !methodName.endsWith( "List" ) && returnTypeName.equals( List.class.getName())) {
            return false;
        }
        if (getterMethod && isProtoBufMethod(method) && !methodName.endsWith( "Map" ) && returnTypeName.equals( Map.class.getName())) {
            return false;
        }
        return getterMethod;
    }
    ...
    public String getPropertyName(ExecutableElement getterOrSetterMethod) {
        ...
        if (isProtoBufMethod(getterOrSetterMethod) && propertyName.endsWith( "List" ) && returnTypeName.equals( List.class.getName())) {
            propertyName = propertyName.substring(0, propertyName.length() - 4);
            return propertyName;
        }
        if (isProtoBufMethod(getterOrSetterMethod) && propertyName.endsWith( "Map" ) && returnTypeName.equals( Map.class.getName())) {
